# mojito-sys S6.4 — EpollPoller backend (issue #76, spec §27.1/§28).
...
#
# ReadinessPoller implementation over the frozen mjs_epoll_* C ABI
# (native/include/mojito_sys.h, s6-epoll block; native/posix/mjs_epoll.c).
#
...
#     returns. Delivery is a single memcpy of the raw mjs_epoll_event results —
...
struct EpollPoller(Movable, ReadinessPoller):
    """ReadinessPoller over epoll (spec §28), Linux only.
    ...
    ready fd reports on every wait until drained. EPOLLET / EPOLLONESHOT
    are deliberately NOT exposed through the frozen ABI.
    ...
    def wait_raw(
        ...
        return _externs.probe_epoll_wait(
    ...
    def wait_probe_status(
        ...
        var buf = stack_allocation[MAX_BATCH * 2, Int64]()
        var bp = UnsafePointer[Byte, MutAnyOrigin](unsafe_from_address=Int(buf))
        return _externs.probe_epoll_wait(self.ptr, bp, cap, tslot, ncell)
