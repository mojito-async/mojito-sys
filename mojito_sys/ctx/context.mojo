...
# (native/include/mojito_sys.h, `s5-ctx` block): zero changes to the frozen
# record ABI, one additive dispatch shim (mjs_ctx_call, SYS-2 C-firewall).
# Exposes spec §20's NativeContext surface:
#
...
# address stored in the per-context cookie. The dispatch itself goes through
# the additive C shim mjs_ctx_call(fn, arg) (SYS-2 C-firewall, same pattern as
# the S1 callback lanes): an indirect branch generated by Mojo inline-asm
# crashes the b2 JIT (spike/context_switch/SPIKE_REPORT.md item 5), so the
# call-by-address is done in plain C, never in inline asm.
#
...
# PLATFORM BOUND: aarch64 Mach-O only. The adrp/add idiom and the underscore
# prefix do not assemble on x86-64 or ELF targets; the s5-ctx C block matches
# (x86-64 is documented not-NativeContext-supported).
...
struct NativeContext(Movable):
    ...
    released by `close()`; b2 1.0.0b2 does not run `__deinit__` on locals, so
    drop-time release is not reliable on this toolchain (the `__deinit__`
    below is for conforming compilers). `destroy()` does NOT free them (it
    only poisons the state, matching the frozen contract that destroy "does
    not free the caller's storage"), so `re_capture()` can revive them.
    ...
    def __deinit__(deinit self):
        # b2 1.0.0b2 does not invoke __deinit__ on locals (probe-verified);
        # prefer the explicit close() below on this toolchain.
        if self.live:
            self.record.free()
            self.cookie.free()
            self.live = False
    ...
    def close(self):
        """Release the record and cookie heap blocks. The deterministic
        release path on b2 (NativeSocket.close precedent). The frozen
        contract keeps destroy() free-less so re_capture can revive the
        storage; close() is final."""
        if self.live:
            self.record.free()
            self.cookie.free()
            self.live = False
    ...
    def finish_stamp(self) -> Int:
        """How many completed lifetimes this record has finished (the finish
        trampoline increments it exactly once per completed lifetime).
        Reads 0 on a dead or moved-from value."""
        if not self.live or Int(self.cookie) == 0:
            return 0
        return self.cookie[C_STAMP]
    ...
    def create(
        ...
        if Int(entry_fn) == 0:
            # The frozen C layer rejects a NULL entry, but the wrapper stores
            # the caller entry in the cookie and dispatches later through
            # mjs_ctx_call, so the C check never sees it; reject it here so a
            # bad create raises instead of resuming through a null code
            # address.
            raise_errno(EINVAL_RC)
        var cookie = _alloc_cookie()
        ...
        if rc != 0:
            # init rejected the span: release the blocks we allocated before
            # raising, otherwise every rejected create leaks both (b2 never
            # runs __deinit__ on locals).
            record.free()
            cookie.free()
            raise_errno(rc)
    ...
    def switch(from_context: NativeContext, to_context: NativeContext) raises:
        ...
        Both records stay heap-owned, so passing values by value is safe:
        b2 passes by-value parameters as read-only aliases, the callee never
        takes ownership, and the cookie writes go through the alias."""
        if not from_context.live or Int(from_context.cookie) == 0:
            # Moved-from or default-constructed `from`: writing the cookie
            # below would smash a null pointer, so raise the decoded error
            # instead of segfaulting (wrapper contract: wrapper-visible
            # misuse raises, C traps are only for what the wrapper cannot
            # see). A destroyed-from stays legal: destroy() only poisons the
            # cookie, and capture revives destroyed storage.
            raise_errno(EINVAL_RC)
    ...
    def re_capture(mut self) raises:
        ...
        # destroy() memsets the C record, erasing the finish trampoline that
        # create() registered; revive re-registers it so completion stays
        # observable after a destroy -> re_capture cycle.
        _externs.probe_ctx_set_finish_hook(
            _byte_of(self.record),
            entry_pointer["mjs_ctx_finish"](),
            Int(self.cookie),
        )
