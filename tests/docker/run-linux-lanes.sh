...
#
# WHY THIS EXISTS.  `tests/s6/epoll` and `tests/s6/iouring` exercise backends
# that only exist on Linux.  On a macOS dev host every entry point is the
# -ENOSYS stub and both lanes exit 2 (environment), which is honest but
# proves nothing.  This script gives them a real Linux kernel without asking
# anyone to own a Linux box, and without waiting for CI.
#
...
#   tests/docker/run-linux-lanes.sh epoll           # one lane
#   MOJITO_IO_URING=1 tests/docker/run-linux-lanes.sh iouring
#
# Env:
...
if ! "$DOCKER" build -q -t "$IMAGE" -f "$SCRIPT_DIR/Dockerfile" "$SCRIPT_DIR" >/dev/null 2>&1; then
...
"$DOCKER" run --rm -i $sec \
    -e "LANE=$LANE" \
    -e "MOJITO_IO_URING=${MOJITO_IO_URING:-}" \
