# mojito-sys S6.3 — readiness poller conformance (issue #75, spec §27.1).
#
# Drives the §27.1 ReadinessPoller surface (trait + shared IoEvent/IoInterest
# plumbing in mojito_sys.io.poller / mojito_sys.io.readiness, kqueue backend
# in mojito_sys.io.platform.kqueue over the frozen mjs_poller_* C ABI)
# through the §38.7 common semantic suite AND the §31 MUST list:
#
#   t7_01 register-readable     — pipe reader fires once written; token round
#                                 trip (§31 MUST preserve accurately);
#   t7_02 register-writable     — fresh pipe writer is immediately ready;
#   t7_03 timeout-no-events     — a quiet 5 ms wait returns exactly 0 within
#                                 a hard wall-clock bound;
#   t7_04 multiple-ready        — two written pipes surface together under
#                                 cap=2 with distinct tokens;
#   t7_05 unregister            — delivery stops after unregister even with
#                                 a pending write: stale OS events are NOT
#                                 delivered for removed registrations;
#   t7_06 duplicate-register    — re-registration UPSERTS: last interests +
#                                 token win (documented s6-poller contract);
#   t7_07 modify                — interest changes apply live (R -> R|W)
#                                 with the new token;
#   t7_08 close-while-registered— closing BOTH ends under an active
#                                 registration retires the knotes silently:
#                                 no event, no crash, poller stays usable
#                                 (§31 close-while-registered);
#   t7_09 descriptor-reuse      — a NEW pipe reusing the closed fd number
#                                 registers cleanly; events carry the CURRENT
#                                 registration's token, never the dead one
#                                 (§38.7 close followed by reuse);
#   t7_10 token-reuse           — unregister + re-register of the SAME fd
#                                 with a different token delivers only the
#                                 new token (§31 token reuse);
#   t7_11 eof                   — writer close delivers READABLE|EOF exactly
#                                 once (kqueue EVFILT_READ EV_EOF mapping);
#   t7_12 error-hangup          — SO_LINGER{1,0}+close RST surfaces the
#                                 ERROR flag on the registered reader;
#   t7_13 interrupt-retry       — SIMULATION of the wait-status mapping core
#                                 (precedent t6_09): rc 0 -> ok, -EINTR ->
#                                 interrupted (caller retries), other -> error,
#                                 plus a live retry-loop shape over timeouts;
#   t7_14 wake-blocked-waiter   — a thread parked in wait(None) returns 0
#                                 events promptly after wake(); wake itself
#                                 never parks (SYS-5 trio: wait blocks,
#                                 wake doesn't);
#   t7_15 wake-sticky           — a wake with NO waiter makes exactly one
#                                 later immediate wait return promptly with
#                                 zero events (EVFILT_USER stickiness);
#   t7_16 racing-unregister     — a writer thread spamming one pipe while
...
#                                 unregister);
#   t7_17 concurrent-control    — four threads run concurrent control-op
#                                 cycles on their own pipes while main waits:
#                                 all report clean rcs (§31 concurrent
#                                 control operations);
#   t7_18 scale-tiers           — 1k/10k/100k registrations where host fd
#                                 limits permit; skipped tiers print an
#                                 explicit caveat line (spec L1917);
#   t7_19 destruction           — close() consumes the handle; double close
#                                 and any post-close op raise -EINVAL.
#
# Trait conformance proof: t7_01-t7_12 drive KqueuePoller through generic
...
from mojito_sys.io.externs import (
    ...
    TimeoutSlot,
    WaitCountSlot,
    probe_poller_register,
    probe_poller_unregister,
    probe_poller_wait,
    probe_poller_wake,
)
...
from mojito_sys.io.platform.kqueue import (
    WAIT_ERROR,
    WAIT_INTERRUPTED,
    WAIT_OK,
    KqueuePoller,
...
@export("mjs_plr_t7_wake_wait_entry")
...
@export("mjs_plr_t7_writer_entry")
...
def _run_scale_tier(mut p: KqueuePoller, tier: Int) raises -> Bool:
...
def _teardown_tier(mut p: KqueuePoller, fds: Int32Ptr, made: Int) raises -> Bool:
    var ok = True
    var j = 0
    while j < made:
...
def main() raises:
    ...
    _drain_one(p1[0])
    if not check("t7_01 register-readable(token-round-trip)", e1_ok):
        failed += 1

    # ======================= t7_02 register writable (immediate readiness) ===
    ...
    if not check("t7_02 register-writable(immediate)", e2_ok):
        failed += 1

    # ======================= t7_03 timeout with no events ====================
    ...
    if not check("t7_03 timeout-no-events(5ms)", e3_ok):
        failed += 1

    # ======================= t7_04 multiple ready handles ====================
    ...
    _drain_one(pa[0])
    _drain_one(pb[0])
    if not check("t7_04 multiple-ready-handles", e4_ok):
        failed += 1

    # ======================= t7_05 unregister stops delivery =================
    ...
    _drain_one(p8b[0])
    if not check("t7_08 close-while-registered(silent-retire)", e8_ok):
        failed += 1

    # ======================= t7_09 descriptor reuse after close ==============
    ...
    if not check("t7_09 descriptor-reuse(current-token-routes)", e9_ok):
        failed += 1

    # ======================= t7_10 token reuse on the same fd ================
    ...
    _t_unregister(poller, p10[0])
    _drain_one(p10[0])
    if not check("t7_10 token-reuse(new-token-only)", e10_ok):
        failed += 1
    ...
    if not check("t7_11 eof(readable|eof-once)", e11_ok):
        failed += 1

    # ======================= shared loopback fixture for t7_12 ===============
    ...
    if not check("t7_13 interrupt-retry(mapping-core)", e13_ok):
        failed += 1
    ...
    var wk_rc = probe_poller_wake(poller.handle_ptr())
    ...
    if not check("t7_14 wake-unblocks-blocked-waiter", e14_ok):
        failed += 1

    # ======================= t7_15 sticky wake before wait ===================
    ...
    # ======================= t7_16 readiness racing unregister ===============
    ...
    if not check("t7_16 racing-unregister(live-token-only)", e16_ok):
        failed += 1

    # ======================= t7_17 concurrent control operations =============
    var cc_ok = True
    var threads = 4
    ...
    if not check("t7_17 concurrent-control-ops(4-threads)", cc_ok):
        failed += 1

    # ======================= t7_18 registration scale tiers ==================
    var tiers_ok = True
    ...
    if not _run_scale_tier(poller, 100000):
        tiers_ok = False
    if not check("t7_18 scale-tiers(1k/10k/100k-or-caveat)", tiers_ok):
        failed += 1

    # ======================= t7_19 poller destruction ========================
    ...
    try:
        ...
        destroy_ok = destroy_ok and contains(String(e), "EINVAL")
    if not check("t7_19 destruction(consume+double-close-einval)", destroy_ok):
        failed += 1
