#!/bin/sh
# mojito-sys S3.3 — atomic wait/wake conformance lane runner (issue #59).
#
# Builds libmojito_sys.dylib (the Makefile picks up native/posix/*.c by
# wildcard), then drives BOTH layers of the s3-atomic-wait surface:
#   - s3-atomic-wait-mojo   — the backend-parameterized §18 conformance
#     (conformance.mojo, AOT-built per the b2 note in tests/s2/thread);
#   - s3-atomic-wait-c      — the raw C smoke (atomic_wait_smoke.c,
#     linked against the packaged dylib so a missing export is a hard
#     failure);
#   - s3-atomic-wait-import — package-path import check for
#     mojito_sys.sync.atomic_wait.
#
# TEST_NAME is "s3-atomic-wait" (the precommit/known-red.tsv key while the
# lane is TDD-red). Prints '<row> PASS/FAIL' lines; exits nonzero when any
# row fails. The mojo row requires zero ": FAIL" verdicts AND a final
# "RESULT: n/n PASSED" line (both suite modes print that shape).
#
# Usage: tests/s3/sync/atomic_wait/run.sh    MOJO=/path/to/mojo
...
TEST_NAME="s3-atomic-wait"
TEST_FILE="$SCRIPT_DIR/conformance.mojo"
SMOKE_FILE="$SCRIPT_DIR/atomic_wait_smoke.c"
...
# current native sources (mjs_atomic_wait.c included once it lands).
...
BIN="$BUILD_DIR/atomic_wait_conformance"
status=2
out=""
attempt=0
...
if [ $status -eq 0 ] \
    ...
    matrix="${matrix}s3-atomic-wait-mojo PASS
"
else
    matrix="${matrix}s3-atomic-wait-mojo FAIL
"
fi
...
if command -v "$CC" >/dev/null 2>&1; then
    CBIN="$BUILD_DIR/atomic_wait_smoke"
...
printf '%s\n' "$cout" | sed 's/^/   | /'
if [ $cstatus -eq 0 ]; then
    matrix="${matrix}s3-atomic-wait-c PASS
"
else
    matrix="${matrix}s3-atomic-wait-c FAIL
"
fi
...
echo "== s3-atomic-wait-import"
printf '%s\n' "$imp_out" | sed 's/^/   | /'
if [ $imp_status -eq 0 ] && printf '%s' "$imp_out" | grep -q "s3-atomic-import-ok"; then
    matrix="${matrix}s3-atomic-wait-import PASS
"
else
    matrix="${matrix}s3-atomic-wait-import FAIL
"
fi

echo ""
echo "S3.3 sync atomic wait/wake conformance matrix (issue #59)"
