#!/bin/sh
# mojito-sys S3.5 — NativeEvent conformance lane runner (issue #61).
#
# Builds libmojito_sys.dylib (the Makefile picks up native/posix/*.c by
# wildcard), then drives BOTH layers of the s3-event surface:
#   - s3-event-mojo   — the §17 NativeEvent wrapper conformance
#     (conformance.mojo, AOT-built per the b2 note in tests/s2/thread);
#   - s3-event-c      — the raw C smoke (event_smoke.c, linked against
#     the packaged dylib so a missing export is a hard failure);
#   - s3-sync-import  — package-path import check for the event
#     surface of mojito_sys.sync.
#
# TEST_NAME is "s3-event" (the precommit/known-red.tsv key while the
# lane is TDD-red). Prints '<row> PASS/FAIL' lines plus "RESULT: all
# green"; exits nonzero when any row fails.
#
# Usage: tests/s3/sync/event/run.sh    MOJO=/path/to/mojo
...
SMOKE_FILE="$SCRIPT_DIR/event_smoke.c"
...
# current native sources (mjs_event.c included).
...
BIN="$BUILD_DIR/event_conformance"
status=2
out=""
attempt=0
...
if [ $status -eq 0 ] && printf '%s' "$out" | grep -q "RESULT: 8/8 PASSED"; then
    matrix="${matrix}s3-event-mojo PASS
"
else
    matrix="${matrix}s3-event-mojo FAIL
"
fi
...
printf '%s\n' "$cout" | sed 's/^/   | /'
if [ $cstatus -eq 0 ]; then
    matrix="${matrix}s3-event-c PASS
"
else
    matrix="${matrix}s3-event-c FAIL
"
fi
...
echo "== s3-sync-import"
printf '%s\n' "$imp_out" | sed 's/^/   | /'
if [ $imp_status -eq 0 ] && printf '%s' "$imp_out" | grep -q "event-import-ok"; then
...
echo ""
echo "S3.5 sync event conformance matrix (issue #61)"
