#!/bin/sh
# mojito-sys S3.7 — NativeSemaphore conformance lane runner (issue #106).
#
# Builds libmojito_sys.dylib (the Makefile picks up native/posix/*.c by
# wildcard), then drives BOTH layers of the s3-semaphore surface:
#   - s3-semaphore-mojo  — the §17 NativeSemaphore wrapper conformance
#     (conformance.mojo, AOT-built per the b2 note in tests/s2/thread);
#   - s3-semaphore-c     — the raw C smoke (sem_smoke.c, linked
#     against the packaged dylib so a missing export is a hard failure);
#   - s3-sync-import     — package-path import check for the semaphore
#     surface of mojito_sys.sync.
#
# TEST_NAME is "s3-semaphore" (the precommit/known-red.tsv key while
# the lane is TDD-red). Prints '<row> PASS/FAIL' lines plus "RESULT:
# all green"; exits nonzero when any row fails.
#
# Usage: tests/s3/sync/semaphore/run.sh    MOJO=/path/to/mojo
...
TEST_NAME="s3-semaphore"
TEST_FILE="$SCRIPT_DIR/conformance.mojo"
SMOKE_FILE="$SCRIPT_DIR/sem_smoke.c"
...
# current native sources (mjs_sem.c included once it lands).
...
BIN="$BUILD_DIR/semaphore_conformance"
status=2
out=""
attempt=0
...
if [ $status -eq 0 ] && printf '%s' "$out" | grep -q "RESULT: 8/8 PASSED"; then
    matrix="${matrix}s3-semaphore-mojo PASS
"
else
    matrix="${matrix}s3-semaphore-mojo FAIL
"
fi
...
if command -v "$CC" >/dev/null 2>&1; then
    CBIN="$BUILD_DIR/semaphore_smoke"
...
printf '%s\n' "$cout" | sed 's/^/   | /'
if [ $cstatus -eq 0 ]; then
    matrix="${matrix}s3-semaphore-c PASS
"
else
    matrix="${matrix}s3-semaphore-c FAIL
"
fi
...
echo "== s3-sync-import"
printf '%s\n' "$imp_out" | sed 's/^/   | /'
if [ $imp_status -eq 0 ] && printf '%s' "$imp_out" | grep -q "semaphore-import-ok"; then
...
echo ""
echo "S3.7 sync semaphore conformance matrix (issue #106)"
