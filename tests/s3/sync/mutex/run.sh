#!/bin/sh
# mojito-sys S3.1 — NativeMutex conformance lane runner (issue #57).
#
# Builds libmojito_sys.dylib (the Makefile picks up native/posix/*.c by
# wildcard), then drives BOTH layers of the s3-mutex surface:
#   - s3-mutex-mojo  — the §15 NativeMutex wrapper conformance
#     (conformance.mojo, AOT-built per the b2 note in tests/s2/thread);
#   - s3-mutex-c     — the raw C smoke (mutex_smoke.c, linked against the
#     packaged dylib so a missing export is a hard failure);
#   - s3-sync-import — package-path import check for mojito_sys.sync.
#
# TEST_NAME is "s3-mutex" (the precommit/known-red.tsv key while the lane
# is TDD-red). Prints '<row> PASS/FAIL' lines plus "RESULT: all green";
# exits nonzero when any row fails.
#
# Usage: tests/s3/sync/mutex/run.sh    MOJO=/path/to/mojo
...
SMOKE_FILE="$SCRIPT_DIR/mutex_smoke.c"
...
# current native sources (mjs_mutex.c included once it lands).
...
BIN="$BUILD_DIR/mutex_conformance"
status=2
out=""
attempt=0
...
printf '%s\n' "$out" | sed 's/^/   | /'
if [ $status -eq 0 ] && printf '%s' "$out" | grep -q "RESULT: 6/6 PASSED"; then
    matrix="${matrix}s3-mutex-mojo PASS
"
else
    matrix="${matrix}s3-mutex-mojo FAIL
"
fi
...
printf '%s\n' "$cout" | sed 's/^/   | /'
if [ $cstatus -eq 0 ]; then
    matrix="${matrix}s3-mutex-c PASS
"
else
    matrix="${matrix}s3-mutex-c FAIL
"
fi
...
echo "== s3-sync-import"
printf '%s\n' "$imp_out" | sed 's/^/   | /'
if [ $imp_status -eq 0 ] && printf '%s' "$imp_out" | grep -q "sync-import-ok"; then
...
echo ""
echo "S3.1 sync mutex conformance matrix (issue #57)"
