#!/bin/sh
# t14_runtime_audit.sh — S5.5 conformance T14: Mojo runtime independence of
# the packaged libmojito_sys.dylib (issue #68). Non-vacuous by construction
# (fixes the PR#21-era "nm -uU no-op" vacuity):
#   1. plain `nm -u` collects the undefined set;
#   2. VACUITY GUARD — FAIL unless the set is non-empty AND contains a known
#      libc/dyld anchor, proving the audit sees real imports;
#   3. EXPECTED-ABSENCE — zero Modular/Mojo private runtime symbols may
#      appear in the undefined OR the exported set;
#   4. internals discipline — the asm bookkeeping/helper privates must stay
#      out of the export table.
#
# Usage: tests/s5/ctx/conformance/t14_runtime_audit.sh (exit 0 = clean)
...
[ -f "$DYLIB" ] || { echo "T14: $DYLIB missing; run \`make\` first"; exit 2; }
command -v nm >/dev/null 2>&1 || { echo "T14 ERROR: nm unavailable"; exit 2; }

TMP=$(mktemp -d) || exit 2
trap 'rm -rf "$TMP"' EXIT

echo "== T14 Mojo runtime independence (non-vacuous audit)"
echo "auditing $DYLIB"
...
if [ ! -s "$TMP/undef" ]; then
    ...
    failures=$((failures + 1))
elif ! grep -Eq '^(memset|memcpy|mmap|mprotect|malloc|sysconf|dyld|pthread)' \
        "$TMP/undef"; then
    echo "  AUDIT FAIL: undefined set lacks any known libc/dyld anchor;"
    echo "    the import view is unreliable (vacuous-audit guard)"
    failures=$((failures + 1))
else
    echo "  vacuity guard (undefined set non-empty, libc/dyld anchor)  OK"
fi
...
if [ -n "$forbidden_hits" ]; then
    ...
    echo "  expected-absence: 0 Modular-runtime symbols (undef+exported)  OK"
fi
