...
```sh
mojo run -I ../../spike/context_switch benchmark/spike/bench_switch.mojo
```

or `make bench` from the repo root once the foundation Makefile (#8) defines
the target.
...
> **PLACEHOLDER** — populated when the S0 implementation lands and this
> benchmark goes green. Paste the markdown tables the bench prints here,
> one block per machine/run, with CPU model and OS version noted.

```
(not yet run: implementation absent)
```

## Status: RED (TDD)

The bench currently fails deterministically because the implementation it
consumes does not exist yet:

```text
$ mojo run -I spike/context_switch benchmark/spike/bench_switch.mojo
benchmark/spike/bench_switch.mojo:59:6: error: unable to locate module 'mojito_spike'
from mojito_spike import (
     ^
/opt/homebrew/opt/mojolang/libexec/bin/mojo: error: failed to parse the provided Mojo source module
```

This is commit 1 of the TDD flow: red tests first, green implementation
after the central merges.
